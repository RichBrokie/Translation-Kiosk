...
async def main():
    ...
    files = glob.glob('/mnt/models/Mandarin Chinese Talks/*如何*.wav') or glob.glob('/mnt/models/Mandarin Chinese Talks/*太努力*.wav')
    f = sorted(files)[0]
    for sec in [30.0, 45.0, 60.0, 90.0]:
        ...
        print(f'ZH at {sec}s -> Lang: {tr.language} | Text: "{tr.text}"')
        if tr.language == 'zh':
