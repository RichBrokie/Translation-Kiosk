...
- Thread-safe and async-safe PCM AudioRollingBuffer
- Fuzzy SequenceMatcher TextStitcher with boundary word repair
...
class AudioRollingBuffer:
    ...
    async def append_pcm(self, chunk: bytes) -> None:
        """Appends arbitrary PCM bytes into the rolling buffer (async-safe)."""
        if not chunk:
            return
    ...
    def process_window(self, curr_raw_text: str) -> Tuple[str, str, str, int]:
        ...
        if match.size >= 1:
            ...
            if prev_norm and curr_norm and self.is_partial_word_match(prev_norm[-1], curr_norm[0]):
                ...
                # Proportional 50% fallback split
                split_idx = max(1, int(len(curr_words) * self.overlap_ratio))

        # Partition current window into overlap-to-commit and new-tentative-tail
...
class ComparativeEngine:
    ...
    def process_step(
        ...
        whisper_naive_latency_ms: float = 0.0,
...
# ============================================================================
# 5. Integrated Pipeline Data Model & Coordinator
# ============================================================================
@dataclass
class PipelineResult:
    """Full end-to-end result emitted for each processed audio window."""
    raw_text: str             # Transcription from naive chunk or raw window
    window_text: str          # Current 4.0s window transcription
...
class AudioPipeline:
    ...
    async def flush(self) -> Optional[PipelineResult]:
        ...
        if slice_data:
            ...
            is_en = self._last_detected_language in ("en", "english")
    ...
    async def _process_window_pcm(self, window_pcm: bytes, is_final: bool = False) -> PipelineResult:
        ...
        source_lang = asr_res.language or "en"
        ...
        # 5. Qwen 2.5 72B Post-Correction & Translation
        text_to_translate = stitched_text if stitched_text else window_raw_text
        qwen_res = await self.qwen_client.post_correct_and_translate(
            text_to_translate,
