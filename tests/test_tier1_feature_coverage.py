...
def test_tc_t1_f04_04_japanese_language_detection():
    ...
    res = TranscriptionResult(text="???????????????", language="ja")
...
def test_tc_t1_f06_02_fuzzy_phonetic_alignment_repair():
    ...
    stitcher = TextStitcher()
    stitcher.process_window("podr??n encontrar una col")
    _, _, full_text, _ = stitcher.process_window("una colecci??n de cartas")
    assert full_text == "podr??n encontrar una colecci??n de cartas"
...
async def test_tc_t1_f07_01_structured_json_translation_request():
    ...
    mock_resp = httpx.Response(200, json={
        "choices": [{
            "message": {
                "content": '{"corrected_text": "En el segundo piso podr??n encontrar cartas.", "english_translation": "On the second floor you will find letters."}'
            }
        }]
    })
    ...
    with patch.object(client._client, "post", new_callable=AsyncMock) as mock_post:
        ...
        assert res.corrected_text == "En el segundo piso podr??n encontrar cartas."
...
def test_tc_t1_f07_03_grammatical_correction_verification():
    ...
    raw_llm = '{"corrected_text": "ne s\'est jamais donn?? les moyens", "english_translation": "has never given himself the means"}'
    parsed = parse_qwen_json(raw_llm, fallback_text="raw")
    assert parsed["corrected_text"] == "ne s'est jamais donn?? les moyens"
